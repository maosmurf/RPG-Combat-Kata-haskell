...
testDamage (before, amount) expectation =
   ...
   damage (newCharacter "Victor", before, amount) `shouldSatisfy` expectation

testHeal :: (Character, Natural) -> (Character -> Bool) -> Spec
testHeal (before, amount) expectation =
   it (printf "%s -> heal %3d" (show before) amount) $
   heal (newCharacter "Heidi", before, amount) `shouldSatisfy` expectation
...
spec = do
  ...
  describe "Characters can Deal Damage to Characters" $ do
    describe "Damage is subtracted from Health" $ do
      testDamage (newCharacter "Alice", 0) (\Character {health = h, state = s} -> h == 1000 && s == Alive)
      testDamage (newCharacter "Bob", 10) (\Character {health = h, state = s} -> h == 990 && s == Alive)
      testDamage (newCharacter "Charlie", 999) (\Character {health = h, state = s} -> h == 1 && s == Alive)
    describe "When damage received exceeds current Health, Health becomes 0 and the character dies" $ do
      testDamage (newCharacter "Alice", 1000) (\Character {health = h, state = s} -> h == 0 && s == Dead)
      testDamage (newCharacter "Bob", 1001) (\Character {health = h, state = s} -> h == 0 && s == Dead)
  describe "A Character can Heal a Character" $ do
    testHeal (Character {name = "Alice", health = 1, level = 1, state = Alive}, 1) (\Character {health = h, state = s} -> h == 2 && s == Alive)
    testHeal (Character {name = "Bob", health = 1, level = 1, state = Alive}, 999) (\Character {health = h, state = s} -> h == 1000 && s == Alive)
    testHeal (Character {name = "Chrlie", health = 999, level = 1, state = Alive}, 1) (\Character {health = h, state = s} -> h == 1000 && s == Alive)
    it "A dead character cannot heal" $
      evaluate (heal (Character {name = "Alice", health = 0, level = 1, state = Dead}, newCharacter "Bob", 1)) `shouldThrow`
      errorCall "A dead character cannot heal"
    it "A dead character cannot be healed" $
      evaluate (heal (newCharacter "Alice", Character {name = "Bob", health = 0, level = 1, state = Dead}, 1)) `shouldThrow`
